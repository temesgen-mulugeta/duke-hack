...
wss.on("connection", (ws: WebSocket) => {
  ...
  // Handle incoming messages from clients (e.g., canvas user updates)
  ws.on("message", (data: WebSocket.Data) => {
    try {
      const message = JSON.parse(data.toString());
      logger.info("Received WebSocket message:", message.type);

      // Forward canvas user updates to all clients (including Next.js client)
...
app.post("/api/elements/sync", (req: Request, res: Response) => {
  try {
    ...
    logger.info(
      `Sync completed: ${successCount}/${frontendElements.length} elements synced`
    );
...
app.get("/", (req: Request, res: Response) => {
  ...
  res.sendFile(htmlFile, (err) => {
    if (err) {
      ...
      res
        .status(404)
        .send('Frontend not found. Please run "npm run build" first.');
    }
  });
});
