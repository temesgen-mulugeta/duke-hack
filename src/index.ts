...
async function syncToCanvas(
  ...
  try {
    ...
    if (!response.ok) {
      logger.warn(
        `Canvas sync returned error status: ${response.status}`,
        result
      );
      throw new Error(
        result.error ||
...
async function updateElementOnCanvas(
  elementData: Partial<ServerElement> & { id: string }
): Promise<ServerElement | null> {
...
async function batchCreateElementsOnCanvas(
  elementsData: ServerElement[]
): Promise<ServerElement[] | null> {
...
async function getElementFromCanvas(
  elementId: string
): Promise<ServerElement | null> {
  if (!ENABLE_CANVAS_SYNC) {
  ...
  try {
    const response = await fetch(
      `${EXPRESS_SERVER_URL}/api/elements/${elementId}`
    );
    if (!response.ok) {
...
const tools: Tool[] = [
  ...
  },
  {
    ...
    description:
      "Create multiple Excalidraw elements at once - ideal for complex diagrams",
    inputSchema: {
      type: "object",
      properties: {
        elements: {
  ...
  },
  {
    ...
    description:
      "Capture a screenshot of the current Excalidraw canvas to see what the user has drawn",
    inputSchema: {
      type: "object",
      properties: {
        format: {
  ...
  async (request: CallToolRequest) => {
    try {
      ...
      switch (name) {
        ...
        case "update_element": {
          const params = ElementIdSchema.merge(ElementSchema.partial()).parse(
            args
          );
          ...
          // Convert text to label format for Excalidraw
          const excalidrawElement = convertTextToLabel(
            updatePayload as ServerElement
          );
          ...
          if (!canvasElement) {
            throw new Error(
              "Failed to update element: HTTP server unavailable or element not found"
            );
          }
        ...
        case "delete_element": {
          ...
          if (!canvasResult || !(canvasResult as ApiResponse).success) {
            throw new Error(
              "Failed to delete element: HTTP server unavailable or element not found"
            );
          }
        ...
        case "query_elements": {
          ...
          try {
            ...
            throw new Error(
              `Failed to query elements: ${(error as Error).message}`
            );
          }
        }
          ...
          switch (resource) {
            ...
            case "elements":
              try {
                ...
                const response = await fetch(
                  `${EXPRESS_SERVER_URL}/api/elements`
                );
                if (!response.ok) {
                  throw new Error(
                    `HTTP server error: ${response.status} ${response.statusText}`
                ...
                throw new Error(
                  `Failed to get elements: ${(error as Error).message}`
                );
              }
              break;
            case "theme":
        ...
        case "group_elements": {
          ...
          try {
            ...
            const updatePromises = elementIds.map(async (id) => {
              ...
              return await updateElementOnCanvas({
                id,
            ...
            if (successCount === 0) {
              ...
              throw new Error(
                "Failed to group any elements: HTTP server unavailable"
              );
            }

            logger.info("Grouping elements", {
              elementIds,
              groupId,
              successCount,
            });
            ...
            throw new Error(
              `Failed to group elements: ${(error as Error).message}`
            );
          }
        }
        ...
        case "ungroup_elements": {
          ...
          try {
            ...
            const updatePromises = (elementIds ?? []).map(async (id) => {
              ...
              if (!element) {
                logger.warn(
                  `Element ${id} not found on canvas, skipping ungroup`
                );
                return null;
              }
            ...
            if (successCount === 0) {
              logger.warn(
                "Failed to ungroup any elements: HTTP server unavailable or elements not found"
              );
            }

            logger.info("Ungrouping elements", {
              groupId,
              elementIds,
              successCount,
            });
            ...
            throw new Error(
              `Failed to ungroup elements: ${(error as Error).message}`
            );
          }
        }
        ...
        case "lock_elements": {
          ...
          try {
            ...
            if (successCount === 0) {
              throw new Error(
                "Failed to lock any elements: HTTP server unavailable"
              );
            }
            ...
            throw new Error(
              `Failed to lock elements: ${(error as Error).message}`
            );
          }
        }
        ...
        case "unlock_elements": {
          ...
          try {
            ...
            if (successCount === 0) {
              throw new Error(
                "Failed to unlock any elements: HTTP server unavailable"
              );
            }
            ...
            throw new Error(
              `Failed to unlock elements: ${(error as Error).message}`
            );
          }
        }
        ...
        case "create_from_mermaid": {
          ...
          logger.info(
            "Creating Excalidraw elements from Mermaid diagram via MCP",
            {
          ...
          try {
            ...
            const response = await fetch(
              `${EXPRESS_SERVER_URL}/api/elements/from-mermaid`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            throw new Error(
              `Failed to process Mermaid diagram: ${(error as Error).message}`
            );
          }
        }
        ...
        case "batch_create_elements": {
          const params = z
            ...
            .parse(args);
          logger.info("Batch creating elements via MCP", {
          ...
          const canvasElements = await batchCreateElementsOnCanvas(
            createdElements
          );

          if (!canvasElements) {
            throw new Error(
              "Failed to batch create elements: HTTP server unavailable"
            );
          }
            ...
            content: [
              {
                ...
                text: `${
                  ...
                  result.syncedToCanvas
                    ? "✅ All elements synced to canvas"
                    : "⚠️  Canvas sync failed (elements still created locally)"
                }`,
              },
            ],
          };
        ...
        case "capture_canvas_screenshot": {
          ...
          try {
            ...
            const response = await fetch(
              `${EXPRESS_SERVER_URL}/api/canvas/screenshot`,
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
            ...
            return {
              content: [
                ...
                },
                {
                  ...
                  data: result.data,
                  mimeType:
                    params.format === "png" ? "image/png" : "image/svg+xml",
                },
              ],
            };
          } catch (error) {
        ...
        case "clear_canvas": {
          ...
          try {
            ...
            throw new Error(
              `Failed to clear canvas: ${(error as Error).message}`
            );
          }
        }
